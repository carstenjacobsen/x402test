import { withPayment, STELLAR_TESTNET } from "@/lib/x402-server";
...
const DOWNLOAD_DATA = [
  {
    name: "Logo_Small_Black.png",
    description: "Stellar logo in PNG format",
    url: "/Logo_Small_Black.png"
  },
];


async function handler(): Promise<Response> {
  // Rotate through cities based on minute — just for fun
  const image = DOWNLOAD_DATA[Math.floor(Date.now() / 60_000) % DOWNLOAD_DATA.length];
...
// Demo fallback address — replace with your own in .env.local
const DEMO_PAYTO = "GBBD47IF6LWK7P7MDEVSCWR7DPUWV3NY3DTQEVFL4NAT4AQH3ZLLFLA5";
